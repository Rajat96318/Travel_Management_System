...
#include<iomanip> //to show the floating point values
#include<windows.h> // 
using namespace std;

void menu();//main menu function prototype

class ManageMenu  //encapsulation
{
protected:
...
public:
    ...
    ManageMenu() //making constructor(constructor name is always public in c++
    {
    ...
    ~ManageMenu(){} //destructure(destroy the constructor)
};

class Customer
{
public:
    string name, gender, address; //attributes of customer c
    int age, mobileNo, menuBack;
    static int cusID;
    char all[999]; //to get all of the lines

    void getDetails() //to get details from customer
    {
    	//saving these details in file and ios::app is opening file in append mode
    ...
    {
        ifstream in("old-customers.txt"); //to show details from file
        {
            if(!in)
            {
            ...
            in.close(); //close th efile
        }
    }
};

int Customer::cusID; //getting value out of the class

class Cabs  //to book cabs
{
public:
    int cabChoice;
    int kilometers;
    ...
    void cabDetails()
    {
        ...
        if(cabChoice == 1){
            ...
            if (hireCab == 1){
                ...
                cabDetails(); //redirecting back to cabDetails() function
            }
            else{
                cout << "Invalid Input! Redirecting to Previous Menu \nPlease Wait!" << endl;
                Sleep(1100); //wait for 1100 milisecond
                system("CLS");
                cabDetails();
            }
        }
...
float Cabs::lastCabCost; //getting value out of the class
...
public:
    ...
    int packChoice1;
    int gotoMenu;
    static float hotelCost; //static so that we can use it out of the class in inheritance
...
float Booking::hotelCost; //getting hotelCost value out of the class
...
public:
    ...
    void printBill()
    {
    	//saving the receipt in file .txt
...
void menu() //menu function contain main menu //will create objects here
{
...
int main()
{
    ManageMenu startObj; //creating object(constructor bydefault executed by initializing the object)
    return 0;
}
